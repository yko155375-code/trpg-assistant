...
function ensurePlayerCharacterStripV41() {
  ...
  select.innerHTML = state.characters.map((character) => `<option value="${escapePlayerAssetsHtmlV41(character.id)}">${escapePlayerAssetsHtmlV41(character.name)}</option>`).join("");
  if (!state.characters.some((character) => character.id === state.playerCharacterId)) state.playerCharacterId = state.characters[0]?.id || "";
  select.value = state.playerCharacterId || previous || "";
  sceneLabel.textContent = state.mode === "player" ? "目前玩家角色" : "目前場景";
}
...
function assetControlV41(characterId, unit, label, value) {
  return `<div class="asset-control"><button class="text-button" data-asset-unit="${unit}" data-asset-id="${escapePlayerAssetsHtmlV41(characterId)}" data-asset-step="-1" type="button">-</button><div><strong>${value}</strong><span>${label}</span></div><button class="text-button" data-asset-unit="${unit}" data-asset-id="${escapePlayerAssetsHtmlV41(characterId)}" data-asset-step="1" type="button">+</button></div>`;
}
