...
function mergeAssetShadowV41() {
  const shadow = readAssetShadowV41();
  let changed = false;
  state.characters = state.characters.map((character) => {
    const shadowAssets = shadow[character.id];
    if (!shadowAssets) return character;
    if (character.assets?.total === shadowAssets.total) return character;
    changed = true;
    return { ...character, assets: shadowAssets };
  });
...
function readAssetShadowV41() {
  try {
    return JSON.parse(localStorage.getItem("trpg-assistant-assets-v41") || "{}");
  } catch {
    return {};
  }
}
...
function writeAssetShadowV41() {
  ...
  state.characters.forEach((character) => {
    if (character.assets) shadow[character.id] = character.assets;
  });
  localStorage.setItem("trpg-assistant-assets-v41", JSON.stringify(shadow));
}
...
function bindVisibleAssetButtonsV41() {
  document.querySelectorAll("[data-asset-id][data-asset-unit][data-asset-step]").forEach((button) => {
    if (button.dataset.assetBoundV41 === "true") return;
    button.dataset.assetBoundV41 = "true";
...
function patchRemoteAssetMergeV41() {
  ...
  applyRemoteState = function applyRemoteStateWithAssets(remoteState) {
    const localAssetsById = new Map(
      (state.characters || [])
        .filter((character) => character.assets)
        .map((character) => [character.id, character.assets]),
    );

    if (remoteState?.characters && localAssetsById.size) {
      remoteState = {
        ...remoteState,
        characters: remoteState.characters.map((character) => (
          character.assets
            ? character
            : { ...character, assets: localAssetsById.get(character.id) }
        )),
      };
    }
...
(function loadShopV43() {
  if (document.querySelector('script[src*="shop-v43.js"]')) return;
  const script = document.createElement("script");
  script.src = "shop-v43.js?v=43";
  script.defer = true;
  document.body.appendChild(script);
})();

(function loadCharacterAttributesV44() {
  if (document.querySelector('script[src*="character-attributes-v44.js"]')) return;
  const script = document.createElement("script");
  script.src = "character-attributes-v44.js?v=44";
  script.defer = true;
  document.body.appendChild(script);
})();
