...
function ensureViewModeStyles() {
  ...
  style.textContent = `
    body[data-mode="player"] .tabbar{display:grid}
    body[data-mode="player"] .tabbar__button[data-tab="combat"],body[data-mode="player"] .tabbar__button[data-tab="audio"],body[data-mode="player"] #combat,body[data-mode="player"] #audio,body[data-mode="player"] #playerPanel{display:none!important}
    .dice-source-zone{grid-column:1/-1;margin:-4px 0 10px;padding:8px;border:1px solid var(--line);border-radius:8px;background:rgba(18,20,23,.5)}
    .dice-source-zone .dice-source-field{margin-bottom:8px}.dice-source-character{display:grid;gap:6px}.dice-source-character .monitor-card{box-shadow:none}.dice-source-empty{color:var(--muted);font-size:.82rem}
  `;
...
function normalizePlayerView() {
  ...
  document.querySelectorAll(".dm-panel").forEach((panel) => { panel.hidden = false; });
  const playerPanel = document.getElementById("playerPanel");
  if (playerPanel) playerPanel.hidden = true;
  const activePanel = document.querySelector(".tab-panel.is-active");
  if (!activePanel || ["combat", "audio", "notes"].includes(activePanel.id)) activateVisibleTab("dashboard");
}

function activateVisibleTab(tabId) {
  document.querySelectorAll(".tabbar__button").forEach((button) => button.classList.toggle("is-active", button.dataset.tab === tabId));
...
function relocateDiceSourceControl() {
  ...
  if (field.parentElement !== zone) zone.insertBefore(field, zone.firstChild);
}
...
function syncDiceSourceOptions() {
  ...
  select.innerHTML = options.map((name) => `<option value="${escapeViewModeHtml(name)}">${escapeViewModeHtml(name)}</option>`).join("");
...
function renderSelectedDiceCharacter() {
  ...
  target.innerHTML = `<article class="monitor-card"><div class="monitor-card__top"><strong>${escapeViewModeHtml(character.name)}</strong><span>閃避值 ${escapeViewModeHtml(character.evasion)}</span></div><div class="stat-grid"><div class="stat-control"><span>希望骰</span><strong>${character.hopeDice}</strong></div><div class="stat-control"><span>壓力</span><strong>${character.stress}</strong></div><div class="stat-control"><span>護盾槽</span><strong>${character.armor}</strong></div><div class="stat-control"><span>血量</span><strong>${character.hp}</strong></div></div></article>`;
}
