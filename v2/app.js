...
const VERSION_LABEL = "player-current-character-entry-select";
...
function logBoot(phase, details = {}) { console.info("[TRPG v2 boot]", { phase, safeMode: isSafeMode, storageKey: STORAGE_KEY, ...details }); }
function buildSafeModeUrl() { const url = new URL(window.location.href); url.search = ""; url.searchParams.set("safe", "1"); return url.href; }
function backupRawState(rawValue) { if (typeof rawValue !== "string" || !rawValue) return ""; const backupKey = `trpg-assistant-v2-corrupt-backup-${Date.now()}`; window.localStorage.setItem(backupKey, rawValue); return backupKey; }
function currentRawState() { if (typeof bootRawState === "string") return bootRawState; try { return window.localStorage.getItem(STORAGE_KEY) || ""; } catch { return ""; } }
async function exportRawState(statusElement) {
  const raw = currentRawState(); if (!raw) { statusElement.textContent = "目前沒有可備份的 v2 主資料。"; return; }
  try { await navigator.clipboard.writeText(raw); statusElement.textContent = "已將 v2 本機資料複製到剪貼簿。"; }
  catch { const blob = new Blob([raw], { type: "application/json;charset=utf-8" }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `trpg-assistant-v2-backup-${Date.now()}.json`; link.click(); URL.revokeObjectURL(link.href); statusElement.textContent = "已下載 v2 本機資料備份。"; }
}
function renderBootError(error, phase = bootPhase) {
  bootFailed = true; const errorObject = error instanceof Error ? error : new Error(String(error || "未知錯誤"));
  ...
  app.querySelector('[data-boot-action="clear"]')?.addEventListener("click", () => { try { window.localStorage.removeItem(STORAGE_KEY); } catch (clearError) { console.error("[TRPG v2 boot] clear storage failed", clearError); } finally { window.location.href = buildSafeModeUrl(); } });
}
function initializeState() {
  logBoot("start");
  if (isSafeMode) { bootPhase = "safeMode"; state = normalizeState(createDefaultState()); logBoot("normalizeState", { parsed: false, normalized: true, safeMode: true }); return; }
  bootPhase = "loadState"; bootRawState = window.localStorage.getItem(STORAGE_KEY); logBoot("loadState", { hasStoredState: Boolean(bootRawState) });
  let parsedState = createDefaultState();
  if (bootRawState) { bootPhase = "parseStorage"; parsedState = JSON.parse(bootRawState); logBoot("parseStorage", { parsed: true }); } else logBoot("parseStorage", { parsed: false, reason: "empty storage" });
  bootPhase = "normalizeState"; state = normalizeState(parsedState); logBoot("normalizeState", { normalized: true }); bootPhase = "saveState"; state = saveState(state);
}
try { initializeState(); } catch (error) { try { bootBackupKey = backupRawState(bootRawState); } catch (backupError) { console.error("[TRPG v2 boot] corrupt backup failed", backupError); } renderBootError(error, bootPhase); }
...
function normalizeEncounterMonster(monster = {}) { const maxHp = Math.max(1, Number(monster.maxHp ?? monster.hp ?? 1) || 1); const hp = Math.min(maxHp, Math.max(0, Number(monster.hp ?? maxHp) || 0)); const maxStress = Math.max(0, Number(monster.maxStress ?? monster.stress ?? 0) || 0); const stress = Math.min(maxStress, Math.max(0, Number(monster.stress ?? 0) || 0)); return { name: String(monster.name || "未命名怪物").trim() || "未命名怪物", hp, maxHp, stress, maxStress, difficulty: Math.max(0, Number(monster.difficulty ?? 10) || 10), attack: String(monster.attack || monster.attackFormula || "").trim(), damage: String(monster.damage || monster.damageFormula || "").trim(), threshold: String(monster.threshold || "").trim(), notes: String(monster.notes || monster.note || "").trim(), tag: String(monster.tag || "").trim() }; }
function withEncounterMessage(nextState, message) { return { ...nextState, ui: { ...(nextState.ui || {}), encounterTemplateMessage: message } }; }
function saveCurrentEncounter(nextState, name) { const title = String(name || "").trim(); const monsters = Array.isArray(nextState.monsters) ? nextState.monsters : []; if (!title) return withEncounterMessage(nextState, "請先輸入遭遇名稱。"); if (!monsters.length) return withEncounterMessage(nextState, "目前沒有怪物可儲存為遭遇。"); const encounter = { id: `encounter-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, name: title, monsters: monsters.map(normalizeEncounterMonster), createdAt: new Date().toISOString() }; return withEncounterMessage({ ...nextState, encounters: [...normalizeEncounters(nextState.encounters), encounter] }, `已儲存遭遇：${title}`); }
function loadEncounter(nextState, encounterId, mode = "replace") { const encounters = normalizeEncounters(nextState.encounters); const encounter = encounters.find((item) => item.id === encounterId); if (!encounter) return withEncounterMessage(nextState, "找不到遭遇模板。"); let workingState = { ...nextState, encounters, monsters: mode === "append" ? [...(nextState.monsters || [])] : [], ui: { ...(nextState.ui || {}), expandedMonsterId: null } }; encounter.monsters.forEach((monster) => { workingState = addMonster(workingState, { ...monster, hp: monster.hp ?? monster.maxHp ?? 1, stress: monster.stress ?? 0, isDead: false }); }); return withEncounterMessage(workingState, mode === "append" ? `已追加遭遇：${encounter.name}` : `已載入遭遇：${encounter.name}`); }
...
function renderCurrentCharacterBar() {
  const characters = Array.isArray(state.characters) ? state.characters : []; const current = getCurrentCharacter(state);
...
app.addEventListener("click", (event) => {
  const modeButton = event.target.closest("[data-mode]"); if (modeButton) { event.preventDefault(); isDmMenuOpen = false; updateState(setMode(syncFormulaDraft(app), modeButton.dataset.mode)); return; }
  const dmMenuButton = event.target.closest("[data-dm-menu-toggle]"); if (dmMenuButton) { event.preventDefault(); isDmMenuOpen = !isDmMenuOpen; safeRender(); return; }
  const pageButton = event.target.closest("[data-page]"); if (pageButton) { event.preventDefault(); isDmMenuOpen = false; updateState(setActivePage(syncFormulaDraft(app), pageButton.dataset.page)); return; }
  const edgeButton = event.target.closest("[data-roll-edge-mode]"); if (edgeButton) { event.preventDefault(); blurNear(edgeButton); const synced = syncFormulaDraft(edgeButton); const selected = edgeButton.dataset.rollEdgeMode; const current = synced.ui?.rollEdgeMode; updateState({ ...synced, ui: { ...(synced.ui || {}), rollEdgeMode: current === selected ? "" : selected } }); return; }
  const actionButton = event.target.closest("[data-action]"); if (!actionButton) return; event.preventDefault();
...
app.addEventListener("input", (event) => {
  ...
  if (characterId) { if (event.target.dataset.characterField) return saveStateOnly(updateCharacterField(state, characterId, event.target.dataset.characterField, event.target.value)); if (event.target.dataset.statField) return saveStateOnly(updateCharacterStat(state, characterId, event.target.dataset.statField, event.target.value)); if (event.target.dataset.attributeField) return saveStateOnly(updateCharacterAttribute(state, characterId, event.target.dataset.attributeField, event.target.value)); if (event.target.dataset.goldField) return saveStateOnly(updateCharacterGold(state, characterId, event.target.dataset.goldField, event.target.value)); if (event.target.matches("[data-money-field]")) return saveStateOnly(updateCharacterMoney(state, characterId, event.target.value)); }
...
app.addEventListener("submit", (event) => {
  ...
  if (rollForm) { event.preventDefault(); const input = rollForm.querySelector("[data-roll-formula]"); const formula = input?.value || ""; const message = rollForm.parentElement.querySelector("[data-roll-message]"); const actor = rollForm.dataset.rollActor || "玩家"; const key = actorKeyFromElement(rollForm); const synced = setFormulaDraft(state, key, formula); const result = rollFormula(formula); if (!result.ok) { saveStateOnly(synced); if (message) message.textContent = result.error; return; } if (message) message.textContent = ""; const mode = EDGE_MODES.has(synced.ui?.rollEdgeMode) ? synced.ui.rollEdgeMode : ""; const roll = mode ? applyEdge(result, mode) : result; const nextState = mode ? { ...synced, ui: { ...synced.ui, rollEdgeMode: "" } } : synced; return updateState(addRoll(nextState, roll, actor)); }
...
app.addEventListener("wheel", (event) => { const stepper = event.target.closest("[data-number-stepper]"); if (!stepper) return; event.preventDefault(); const delta = event.deltaY < 0 ? 1 : -1; const { characterId, stepperType, stepperField } = stepper.dataset; if (stepperType === "stat") return updateState(adjustCharacterStat(state, characterId, stepperField, delta)); if (stepperType === "attribute") return updateState(adjustCharacterAttribute(state, characterId, stepperField, delta)); if (stepperType === "money") return updateState(adjustCharacterMoney(state, characterId, delta)); if (stepperType === "gold") return updateState(adjustCharacterGold(state, characterId, stepperField, delta)); }, { passive: false });
safeRender();
} catch (error) { try { if (!isSafeMode && !bootBackupKey) bootBackupKey = backupRawState(currentRawState()); } catch (backupError) { console.error("[TRPG v2 boot] event binding backup failed", backupError); } renderBootError(error, "eventBinding"); }
}
