...
function makeMonsterRoll(monster, action, round = null) {
  ...
  if (!String(formula || "").trim()) {
    return { ok: false, skipped: true, action, formula: "", error: `未設定${label}公式`, monsterId: monster.id, monsterName: monster.name, round };
  }

  const result = rollFormula(formula);
  return { ...result, action, monsterId: monster.id, monsterName: monster.name, round, error: result.ok ? "" : `無效${label}公式：${result.error}` };
}
...
export function normalizeMonster(monster = {}) {
  ...
  return {
    id: monster.id || makeId("monster"), name: monster.name || "未命名怪物",
    hp: clamp(toNumber(monster.hp, maxHp), 0, maxHp), maxHp,
    stress: clamp(toNumber(monster.stress), 0, maxStress), maxStress,
    difficulty: Math.max(0, toNumber(monster.difficulty, 10)), attack: normalizeAttackFormula(monster.attack),
    damage: String(monster.damage || "1d6"), threshold: monster.threshold || "", notes: monster.notes || "",
  };
}

export function normalizeMonsters(monsters) { return Array.isArray(monsters) ? monsters.map(normalizeMonster) : []; }

export function addMonster(state, values = {}) {
  const monster = normalizeMonster({ id: makeId("monster"), name: values.name || "新怪物", hp: values.hp, maxHp: values.maxHp, stress: values.stress, maxStress: values.maxStress, difficulty: values.difficulty, attack: values.attack || "d20", damage: values.damage || "1d6", threshold: values.threshold, notes: values.notes });
  return { ...state, monsters: [...normalizeMonsters(state.monsters), monster] };
}

export function updateMonster(state, monsterId, field, value) {
  return { ...state, monsters: normalizeMonsters(state.monsters).map((monster) => monster.id === monsterId ? normalizeMonster({ ...monster, [field]: numberFields.has(field) ? toNumber(value) : value }) : monster) };
}
...
export function adjustMonsterValue(state, monsterId, field, delta) {
  return { ...state, monsters: normalizeMonsters(state.monsters).map((monster) => {
...
export function rollMonsterAction(state, monsterId, action) {
  ...
  return { ...prependRolls(state, [record]), session: { ...state.session, lastMonsterAction: record } };
}

export function advanceMonsterRound(state) {
  const round = Math.max(0, Math.floor(toNumber(state.session?.round))) + 1;
  const results = normalizeMonsters(state.monsters).filter((monster) => monster.hp > 0).map((monster) => ({ id: makeId("monster-round"), monsterId: monster.id, monsterName: monster.name, round, attackRoll: makeMonsterRoll(monster, "attack", round), damageRoll: makeMonsterRoll(monster, "damage", round), time: new Date().toISOString() }));
  const records = results.map((result) => toRollRecord({ ok: true, type: "monster-round", formula: `第 ${round} 回合`, total: result.attackRoll.ok ? result.attackRoll.total : "-", dice: [], terms: [], monsterId: result.monsterId, monsterName: result.monsterName, round, attackRoll: result.attackRoll, damageRoll: result.damageRoll, time: result.time }, "monster round roll", `怪物回合：${result.monsterName}`));
  return { ...prependRolls(state, records), session: { ...state.session, round, monsterRoundResults: results } };
}

export function resetMonsterRound(state) { return { ...state, session: { ...state.session, round: 0, monsterRoundResults: [] } }; }
...
export function renderMonsterManager(state) {
  ...
  return `<section class="monster-panel">${renderRoundPanel(state)}${monsters.length ? `<div class="monster-grid">${monsters.map(renderMonsterCard).join("")}</div>` : `<section class="empty-panel"><strong>目前沒有怪物</strong><p>新增怪物後即可管理戰鬥狀態與回合擲骰。</p></section>`}${expandedMonster ? renderMonsterDetails(expandedMonster) : ""}${renderAddMonsterForm()}</section>`;
}
