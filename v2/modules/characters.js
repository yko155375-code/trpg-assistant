...
export function createCharacter(name = "新角色") {
  return normalizeCharacter({ id: makeId(), name });
}
...
export function deleteCharacter(state, characterId) {
  ...
  return { ...state, characters, ui: { ...state.ui, currentCharacterId: nextCurrent ? nextCurrent.id : null } };
}
...
export function updateCharacterAttribute(state, characterId, field, value) {
  return updateCharacter(state, characterId, (character) => ({
    ...character,
    attributes: { ...character.attributes, [field]: toNumber(value) },
  }));
}
...
export function updateCharacterMoney(state, characterId, value) {
  return updateCharacter(state, characterId, (character) => ({
    ...character,
    assets: { ...character.assets, money: toNumber(value) },
  }));
}
...
export function renderAddCharacterForm() {
  return `
    <form class="inline-form" data-add-character-form>
      <label class="form-field"><span>新增角色</span><input data-new-character-name type="text" placeholder="輸入角色名稱" autocomplete="off" /></label>
...
export function renderCharacterEditor(state, options = {}) {
  ...
  if (!character) {
    return `${renderAddCharacterForm()}<section class="empty-panel"><strong>尚未建立角色</strong><p>新增角色後，就能編輯 HP、壓力、希望、閃避、六屬性與備註。</p></section>`;
  }
  return `
    ...
    <section class="editor-panel" data-character-id="${escapeHtml(character.id)}">
      ...
      <div class="form-grid">${statFields.map((field) => `<label class="form-field"><span>${field.label}</span><input data-character-id="${escapeHtml(character.id)}" data-stat-field="${field.key}" type="number" inputmode="numeric" value="${character.stats[field.key]}" /></label>`).join("")}</div>
      <h4>六屬性</h4>
      <div class="form-grid">${attributeFields.map((field) => `<label class="form-field"><span>${field.label}</span><input data-character-id="${escapeHtml(character.id)}" data-attribute-field="${field.key}" type="number" inputmode="numeric" value="${character.attributes[field.key]}" /></label>`).join("")}</div>
...
export function renderAssetsEditor(state, options = {}) {
  ...
  const character = getCurrentCharacter(state);
  if (!character) {
    return `${showPicker ? renderAddCharacterForm() : ""}<section class="empty-panel"><strong>尚未選擇角色</strong><p>新增或選擇角色後，就能編輯金錢、物品、裝備與消耗品。</p></section>`;
  }
  return `
    ...
    <section class="editor-panel asset-panel" data-character-id="${escapeHtml(character.id)}">
      ...
      <label class="form-field form-field-full"><span>金錢</span><input data-character-id="${escapeHtml(character.id)}" data-money-field type="number" inputmode="numeric" value="${character.assets.money}" /></label>
...
function renderAssetList(character, list) {
  ...
  return `
    <section class="asset-list-card">
      ...
      ${entries.length ? `<ul class="asset-entry-list">${entries.map((entry, index) => `<li><span>${escapeHtml(entry)}</span><button type="button" data-action="delete-asset-entry" data-character-id="${escapeHtml(character.id)}" data-list-key="${list.key}" data-entry-index="${index}">刪除</button></li>`).join("")}</ul>` : `<p class="empty-hint">尚無${list.label}</p>`}
    </section>`;
}
