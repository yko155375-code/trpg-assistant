...
export function ensureCurrentCharacterId(state) {
  ...
  return {
    ...
    ui: { ...state.ui, currentCharacterId: current ? current.id : null },
  };
}
...
export function addCharacter(state, name, color, avatarUrl = "") {
  ...
  return {
    ...
    ui: { ...state.ui, currentCharacterId: nextCharacter.id },
  };
}
...
export function deleteCharacter(state, characterId) {
  ...
  const nextExpanded = state.ui.expandedCharacterId && characters.some((character) => character.id === state.ui.expandedCharacterId) ? state.ui.expandedCharacterId : null;
  return {
    ...state,
    characters,
    ui: { ...state.ui, currentCharacterId: nextCurrent ? nextCurrent.id : null, expandedCharacterId: nextExpanded },
  };
}

export function selectCharacter(state, characterId) {
  return { ...state, ui: { ...state.ui, currentCharacterId: characterId || null } };
}
...
export function updateCharacter(state, characterId, updater) {
  return {
    ...state,
    characters: normalizeCharacters(state.characters).map((character, index) =>
      character.id === characterId ? normalizeCharacter(updater(character), index) : character,
    ),
  };
}
...
export function updateCharacterStat(state, characterId, field, value) {
  return updateCharacter(state, characterId, (character) => ({
    ...character,
    stats: normalizeStats({ ...character.stats, [field]: toNumber(value) }),
  }));
}
...
export function updateCharacterAttribute(state, characterId, field, value) {
  return updateCharacter(state, characterId, (character) => ({
    ...character,
    attributes: normalizeAttributes({ ...character.attributes, [field]: toNumber(value) }),
  }));
}
...
export function updateCharacterMoney(state, characterId, value) {
  ...
  return updateCharacter(state, characterId, (character) => ({
    ...character,
    assets: { ...character.assets, money: goldToHandfuls(gold), gold },
  }));
}
...
export function updateCharacterGold(state, characterId, field, value) {
  ...
  return updateCharacter(state, characterId, (character) => {
    const gold = normalizeGold({ ...character.assets.gold, [normalizedField]: Math.max(0, toNumber(value)) });
    return { ...character, assets: { ...character.assets, money: goldToHandfuls(gold), gold } };
  });
}
...
export function adjustCharacterAttribute(state, characterId, field, delta) {
  const character = normalizeCharacters(state.characters).find((entry) => entry.id === characterId);
  return updateCharacterAttribute(state, characterId, field, toNumber(character?.attributes?.[field]) + toNumber(delta));
}
...
export function toggleCharacterEffect(state, characterId, effectType, value) {
  ...
  return updateCharacter(state, characterId, (character) => {
    ...
    return { ...character, [key]: sortStatusLabels(key, isActive ? entries.filter((entry) => entry !== text) : [...entries, text]) };
  });
}
...
export function deleteAssetEntry(state, characterId, listKey, index) {
  return updateCharacter(state, characterId, (character) => ({
    ...character,
    assets: { ...character.assets, [listKey]: (character.assets[listKey] || []).filter((_, itemIndex) => itemIndex !== index) },
  }));
}
...
export function renderCharacterPicker(state, label = "目前角色") {
  ...
  if (!characters.length) return `<p class="empty-hint">目前沒有角色。請先新增角色。</p>`;
  return `
    <label class="form-field form-field-full">
      <span>${label}</span>
      <select data-character-select>
        ${characters.map((character) => `<option value="${escapeHtml(character.id)}" ${current?.id === character.id ? "selected" : ""}>${escapeHtml(character.name)}</option>`).join("")}
      </select>
    </label>
  `;
}
...
function renderCompactAttributeBadges(character) {
  return `<div class="compact-attribute-grid" aria-label="六大屬性">${attributeFields.map((field) => `<span class="compact-attribute-badge"><b>${field.label}</b><strong>${character.attributes[field.key]}</strong></span>`).join("")}</div>`;
}
...
function renderCompactDebuffChips(character) {
  ...
  return `<div class="compact-debuff-chips" aria-label="${escapeHtml(character.name)}負面狀態">${presets.map((effect) => {
    const isActive = entries.includes(effect.label);
    return `<button class="compact-debuff-chip ${isActive ? "is-active" : ""}" type="button" data-action="toggle-character-effect" data-character-id="${escapeHtml(character.id)}" data-effect-type="debuffs" data-effect-label="${escapeHtml(effect.label)}" aria-pressed="${isActive}">${escapeHtml(effect.label)}</button>`;
  }).join("")}</div>`;
}
  ...
  return `
    <section class="effect-editor">
      ...
      <div class="effect-chip-row" aria-label="${label}快速選擇">
        ${presets.map((effect) => {
          const isActive = entries.includes(effect.label);
          return `<button class="effect-chip ${isActive ? "is-active" : ""}" type="button" data-action="toggle-character-effect" data-character-id="${escapeHtml(character.id)}" data-effect-type="${effectType}" data-effect-label="${escapeHtml(effect.label)}" aria-pressed="${isActive}">${escapeHtml(effect.label)}</button>`;
        }).join("")}
      </div>
      ...
      ${entries.length ? `<ul class="effect-list">${entries.map((entry, index) => `<li><span>${escapeHtml(entry)}</span><button type="button" data-action="delete-character-effect" data-character-id="${escapeHtml(character.id)}" data-effect-type="${effectType}" data-effect-index="${index}">刪除</button></li>`).join("")}</ul>` : `<p class="empty-hint">尚無${label}</p>`}
    </section>
  `;
}
...
function renderCharacterAdvancedEditor(character) {
  ...
  return `
    <div class="character-advanced-content">
      ...
      <div class="effect-grid">${renderEffectEditor(character, "buffs", "增益")}${renderEffectEditor(character, "debuffs", "負面")}</div>
      <label class="form-field form-field-full"><span>角色備註</span><textarea data-character-id="${escapeHtml(character.id)}" data-character-field="notes" rows="4">${escapeHtml(character.notes)}</textarea></label>
      <section class="character-danger-zone">
        <div>
          <strong>危險操作</strong>
          <small>${isDeleteConfirming ? "再按一次刪除才會移除此角色。" : "刪除角色需要二次確認。"}</small>
        </div>
      ...
      ${isDeleteConfirming ? `<p class="character-delete-warning">點擊其他地方、其他角色、頁籤或關閉進階設定會取消確認。</p>` : ""}
    </div>
  `;
}
...
export function renderCharacterEditor(state, options = {}) {
  ...
  if (!character) return `${renderAddCharacterForm()}<section class="empty-panel"><strong>尚未建立角色</strong><p>新增角色後，就能編輯 HP、壓力、希望、閃避、六屬性與備註。</p></section>`;
  return `
